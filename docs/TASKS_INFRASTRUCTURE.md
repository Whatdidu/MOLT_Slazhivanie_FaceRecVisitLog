...
- [x] **TASK-INF-021**: Интегрировать роутер модуля Recognition (после готовности Мансура)
- [x] **TASK-INF-022**: Интегрировать роутер модуля Employees (после готовности Ольги)
- [x] **TASK-INF-023**: Интегрировать роутер модуля Attendance (после готовности Лили)
- [ ] **TASK-INF-024**: Настроить pipeline: Gateway → Recognition → Attendance

## Фаза 7: Деплой и мониторинг

- [x] **TASK-INF-025**: Настроить деплой на VPS сервер
- [x] **TASK-INF-026**: Настроить nginx reverse proxy (опционально)
- [x] **TASK-INF-027**: Добавить мониторинг логов
- [ ] **TASK-INF-028**: Провести нагрузочное тестирование

## Фаза 8: Интеграция с IP-камерой

- [x] **TASK-INF-029**: Реализовать FTP-сервер для приёма снапшотов с камеры
- [x] **TASK-INF-030**: Настроить FTP passive mode для работы через NAT
- [x] **TASK-INF-031**: Добавить FTP настройки в docker-compose.yml
- [x] **TASK-INF-032**: Реализовать обработчик снапшотов (snapshot_handler.py)
- [x] **TASK-INF-033**: Интегрировать распознавание лиц при получении снапшота
- [x] **TASK-INF-034**: Реализовать автоматическую запись посещаемости при распознавании
- [x] **TASK-INF-035**: Настроить физическую IP-камеру для отправки снапшотов по FTP
- [x] **TASK-INF-036**: Добавить FileBrowser для просмотра снапшотов в реальном времени

## Фаза 9: Оптимизация и стабильность

- [x] **TASK-INF-037**: Увеличить пул соединений БД (pool_size=20, max_overflow=30)
- [x] **TASK-INF-038**: Добавить семафор для ограничения параллельной обработки снапшотов
- [x] **TASK-INF-039**: Настроить timezone Europe/Moscow для контейнеров
- [x] **TASK-INF-040**: Изменить метрику сравнения лиц на Euclidean distance
- [x] **TASK-INF-041**: Добавить защиту админки паролем
...
```text
# Корень проекта
├── docker-compose.yml         # Оркестрация App + PostgreSQL (dev)
...
| Фаза 6: Интеграция | 🔄 В процессе | 3/4 |
| Фаза 7: Деплой | 🔄 В процессе | 3/4 |
| Фаза 8: IP-камера | ✅ Выполнено | 8/8 |
| Фаза 9: Оптимизация | ✅ Выполнено | 5/5 |

**Общий прогресс: 39/41 задач (95%)**
